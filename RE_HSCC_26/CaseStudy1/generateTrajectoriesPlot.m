function h = generateTrajectoriesPlot(varargin)
    ...
    p.addParameter('opacity', 1, @isnumeric);
    p.parse(varargin{:});
    options = p.Results;
    ...
    if options.background
        switch options.case
            ...
            case 'overtake'
                ...
                if options.tikz
                    % Borders
                    tikz = [tikz, sprintf(['  ' '\\draw[black,line width = 2pt] (axis cs:%0.4g,%0.4g) -- (axis cs:%0.4g,%0.4g);' '\n'], 0, params.lane, 30, params.lane)];
                    tikz = [tikz, sprintf(['  ' '\\draw[black,line width = 2pt] (axis cs:%0.4g,%0.4g) -- (axis cs:%0.4g,%0.4g);' '\n'], 0, -2*params.lane, 30, -2*params.lane)];
                    % Lane markation
                    tikz = [tikz, sprintf(['  ' '\\addplot[domain=%0.4g:%0.4g,color=black,line width = 3pt, dashed] {0};' '\n'], 0, 30)];
    ...
    for j=length(objects):-1:1
        if isempty(options.opacity)
            opacity = 10/length(trajectories.(objects{j}.type));
        else
            opacity = options.opacity;
        end
        if opacity ~= 1 % gray foundation for opaque lines
            for i=1:length(trajectories.(objects{j}.type))
                if isempty(options.highlight) || all(i ~= options.highlight) % Only plot non-highlighted trajectories
                    if options.matlab
                        plot(trajectories.(objects{j}.type){i}(1,1:N+1), trajectories.(objects{j}.type){i}(2,1:N+1), 'Color', [0.8 0.8 0.8],  'LineWidth', 1);
                    end
                end
            end
        end
        for i=1:length(trajectories.(objects{j}.type))
            if isempty(options.highlight) || all(i ~= options.highlight) % Only plot non-highlighted trajectories
                if options.matlab
                    p = plot(trajectories.(objects{j}.type){i}(1,1:N+1), trajectories.(objects{j}.type){i}(2,1:N+1), 'Color', colors.(objects{j}.type),  'LineWidth', 1);
                    if opacity ~= 1
                        p.Color(4) = opacity;
                    end
                end
    ...
    if options.tikz
        if ~isempty(options.spy)
            ...
            angle = 225;
            mulpos = [cos(angle/360*2*pi) -sin(angle/360*2*pi); sin(angle/360*2*pi) cos(angle/360*2*pi)]*[options.spy.size; 0]+options.spy.viewer';
            mulpos = mulpos/1.73*10*3.75/options.imgwidth;
            mulpos = mulpos+[options.dims(1); options.dims(3)];
            tikz = [tikz, sprintf(['  ' '\\node[' options.spy.color ',draw,thick,circle,inner sep=0pt] at (axis cs:%0.4g,%0.4g) {' num2str(options.spy.magnification) 'x};' '\n'], mulpos)];
        end
